...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
  // MACRODOWN only works in this function
      switch(id) {
        ...
        if (record->event.pressed) {
          return MACRO( D(LSHIFT), T(COMMA), U(LSHIFT), T(MINS), T(SPC), END  ); // this sends the string '<-' when the macro executes
        }
        break;

        case 2: // this would trigger when you hit a key mapped as M(2)
        if (record->event.pressed) {
          return MACRO( D(LGUI), D(LSHIFT), T(LBRC), U(LGUI), U(LSHIFT), END  ); // this sends the command Cmd + Shift + [ when the macro executes
        }
        break;

        case 3: // this would trigger when you hit a key mapped as M(3)
        if (record->event.pressed) {
          return MACRO( D(LGUI), D(LSHIFT), T(RBRC), U(LGUI), U(LSHIFT), END  ); // this sends the command Cmd + Shift + ] when the macro executes
        }
        break;
      }
    return MACRO_NONE;
